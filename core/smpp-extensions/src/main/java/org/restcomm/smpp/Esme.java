...
public class Esme extends SslConfigurationWrapper implements XMLSerializable, EsmeMBean {
    ...
    /**
     * Set the amount of time(ms) to wait for an endpoint to respond to a request before it expires. Defaults to disabled (-1).
     */
    private long requestExpiryTimeout;

    /**
     * Sets the amount of time between executions of monitoring the window for requests that expire. It's recommended that this
     * generally either matches or is half the value of requestExpiryTimeout. Therefore, at worst a request would could take up
     * 1.5X the requestExpiryTimeout to clear out.
     */
    private long windowMonitorInterval;

    /**
     * Set the amount of time to wait until a slot opens up in the sendWindow. Defaults to 60000.
     */
    ...
    /**
     * @param clientBindTimeout the clientBindTimeout to set
     */
    ...
    /**
     * @param serverBound the server status to set
     */
    ...
    public boolean checkLinkRecvMessage() {
        ...
        if (this.rxEnquireLinkCounter < currentEnqLinkCounter || this.rxSubmitSmRespCount < currentSubmitSmRespCounter
                || this.rxDlrRespCounter < currentDlrRespCounter || this.rxDataSmRespCounter < currentDataSmRespCounter) {
    ...
    protected static final XMLFormat<Esme> ESME_XML = new XMLFormat<Esme>(Esme.class) {
        ...
        public void read(javolution.xml.XMLFormat.InputElement xml, Esme esme) throws XMLStreamException {
            ...
            esme.wrappedSslConfig.setKeyManagerFactoryAlgorithm(xml.getAttribute(KEY_MANAGER_FACTORY_ALGORITHM, "SunX509"));
            ...
            esme.wrappedSslConfig.setTrustManagerFactoryAlgorithm(xml.getAttribute(TRUST_MANAGER_FACTORY_ALGORITHM, "PKIX"));
    ...
    public void show(StringBuffer sb) {
        sb.append(SmppOamMessages.SHOW_ESME_NAME).append(this.name).append(SmppOamMessages.SHOW_ESME_SYSTEM_ID)
                ...
                .append(SmppOamMessages.NORMAL_THRESHOLD).append(this.normalThreshold).append(SmppOamMessages.OVERLOADED)
                .append(this.overloaded).append(SmppOamMessages.SPLIT_LONG_MESSAGES).append(this.splitLongMessages);
    ...
    public void close() {
        if (this.defaultSmppSession != null) {
            try {
                ...
                logger.error(
                        String.format("Failed to close smpp session for %s.", defaultSmppSession.getConfiguration().getName()));
            }
        }
    }
    ...
    /**
     * On receiving of every message from this ESME this method should be invoked. This method updates a counter of received
     * messages and checks if a seconds / minute / hour / day limits are exceeded
     * 
     * @param count received messages count
    ...
    public CheckMessageLimitResult onMessageReceived(int count) {
        ...
        if (rateLimitPerSecond > 0 && cntSecond > rateLimitPerSecond) {
            ...
            if (this.extraMsgPerSecond.addAndGet(1) == 1)
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.firstFault,
                        "RateLimitPerSecond is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perSecond);
            else
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.nextFault,
                        "RateLimitPerSecond is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perSecond);
        }
        ...
        if (rateLimitPerMinute > 0 && cntMinute > rateLimitPerMinute) {
            ...
            if (this.extraMsgPerMinute.addAndGet(1) == 1)
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.firstFault,
                        "RateLimitPerMinute is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perMinute);
            else
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.nextFault,
                        "RateLimitPerMinute is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perMinute);
        }
        ...
        if (rateLimitPerHour > 0 && cntHour > rateLimitPerHour) {
            ...
            if (this.extraMsgPerHour.addAndGet(1) == 1)
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.firstFault,
                        "RateLimitPerHour is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perHour);
            else
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.nextFault,
                        "RateLimitPerHour is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perHour);
        }
        ...
        if (rateLimitPerDay > 0 && cntDay > rateLimitPerDay) {
            ...
            if (this.extraMsgPerDay.addAndGet(1) == 1)
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.firstFault,
                        "RateLimitPerDay is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perDay);
            else
                return new CheckMessageLimitResult(CheckMessageLimitResult.Result.nextFault,
                        "RateLimitPerDay is exceeded for ESME=" + this.name, CheckMessageLimitResult.Domain.perDay);
        }
    ...
    public void setOverloadThreshold(int overloadThreshold) {
        ...
        if (overloadThreshold > 0) {
            if (normalThreshold >= 0 && overloadThreshold > normalThreshold) {
                update = true;
            }
        } else if (overloadThreshold == -1) {
            update = true;
        }
        if (update) {
            this.overloadThreshold = overloadThreshold;
    ...
    public void setNormalThreshold(int normalThreshold) {
        ...
        if (normalThreshold >= 0) {
            if ((overloadThreshold > 0 && overloadThreshold > normalThreshold) || overloadThreshold == -1) {
                update = true;
            }
